...
def run(args: DictConfig):
    ...
    checkpoint_filepath = os.path.join(logdir, 'best.model.keras')

    checkpoint_callback = ModelCheckpoint(
        filepath=checkpoint_filepath,
        monitor='val_loss', # 監視する指標。検証損失を監視
        save_best_only=True, # 最良のモデルのみを保存
        save_weights_only=False, # モデル全体を保存するか、重みだけを保存するかを指定
        mode='min', # 監視する指標の最小値を基準にモデルを保存
        verbose=1 # チェックポイントが保存されたときにメッセージを表示するかどうか
    )
    
    # モデルの訓練（検証データを指定）
    cprint("Training Start", "cyan")
    model.fit(x_train, y_train, 
            epochs=1,  # エポック数を指定します
            batch_size=128,  # バッチサイズを指定します
            validation_data=(x_val, y_val),
            callbacks=[checkpoint_callback])  # 検証データを指定します
    ...
    # モデルの予測
    model = tf.keras.models.load_model(checkpoint_filepath, custom_objects={'custom_loss': custom_loss})
