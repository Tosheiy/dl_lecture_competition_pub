...
def pre_process(data, type):
    ...
    for batch_data in tqdm(dataloader, desc=f"{type}_PreProcess", leave=False):
        data = batch_data.to('cpu').detach().numpy().astype(np.float64).copy()
        ...
        #         # np.clip(data[i, j, :], -3, 3, out=data[i, j, :])  # 範囲外の値をクリッピング
        # 各サンプルの各チャネルをスケーリング


        all_samples.append(data_normalized.astype(np.float32))
